...
<?php foreach($category as $key => $child) : ?>

	<!-- Skip if child havn't element -->
	<?php if (!isset ($child['element'])) {
		continue;
	} else {
		$elementNumber++;
	} ?>
	...
	<?php if ($child['element']['element_id'] == -1) { ?>
		<div style="margin-left:20px;">
		<strong style="text-decoration: underline">
			<?php echo $child['element']['name']; ?>
		</strong>
		<p class ="print-elements">
			<?php CWidget::createWidget('application.modules.doctors.components.widgets.printCategory', array(
	...
	<?php } else { ?>
		...
		<?php if (($element = $child['element']) == null || !($element['value'] != ' ' && $element['value'] != null)) {
			continue;
		} ?>

		<!-- If element's type is 4 (table), then print table -->
		<?php if ($element['type'] == '4') {
			...
			for($i = 0; $i < $configOfTable['numRows']; $i++) {
				...
				for ($j = 0; $j < $configOfTable['numCols']; $j++) :
					...
					<?php endif; ?>
					<td style="border: 1px solid #000000;" class="content-<?php echo $i . '_' . $j; ?>">
						<?php echo $value;?>
					</td>
					</tr>
				<?php endfor;
