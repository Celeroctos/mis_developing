...
//    public $layout = 'application.modules.admin.views.layouts.index';
    ...
    public function actionGetBufferHistoryGreetings() {
        $this->getHistoryGridByModel(new TasuGreetingsBufferHistory());
    }
    ...
    public function actionImportGreetings() {
        ...
        $currentGreeting = isset($_GET['currentGreeting']) && $_GET['currentGreeting'] != null ? $_GET['currentGreeting'] : false;
	...
	//	var_dump(Yii::app()->user->getState('tasuLog', -1));
        ...
        if(count($moreBuffer) == 0) {
			// Найдём все невыгруженные записи (ошибки. Это количество отнять от общего)
			$numErrors = TasuGreetingsBuffer::model()->count(
				'import_id = :import_id
				AND status = :status',
				array(
					':import_id' => $importId,
            ...
            $historyBuffer->num_rows = $this->totalRows - $numErrors;
			...
			// Обновим importId для невыгруженных
			TasuGreetingsBuffer::model()->updateAll(
				array('import_id' => $importId + 1),
				'import_id = :import_id
				AND status = :status',
				array(
					':import_id' => $importId,
    ...
    private function moveGreetingToTasuDb($greeting) {
        ...
        $patients = $this->searchTasuPatient($oms);
        ...
        if(count($patients) == 0 || !$patients) {
			...
			if($medcardRow != null && $medcardRow['number_50713'] != $medcard->card_number) { // Сработало условие, при котором надо перерегистрировать карту
                $this->updateMedcardInTasu($medcard, $patients[0]['PatientUID']);
			} elseif($medcardRow == null) {
                $this->createMedcardInTasu($medcard, $patients[0]['PatientUID']);
			}
		}
        ...
        if(count($patients) > 0) {
            ...
            if($tap instanceof TasuTap) {
                ...
                $this->log[] = '<strong class="text-danger">[Ошибка]</strong> Невозможно добавить приём с ID'.$greeting['greeting_id'].' (карта '.$medcard->card_number.', дата '.$greeting['patient_day'].') в базу: возможно, полис пациента (номер '.$oms->oms_series.' '.$oms->oms_number.') записан в неправильном формате...?.';
				$this->logStr .= "[Ошибка] Невозможно добавить приём с ID".$greeting['greeting_id']." (карта ".$medcard->card_number.", дата ".$greeting['patient_day'].") в базу: возможно, полис пациента (номер ".$oms->oms_series.' '.$oms->oms_number.") записан в неправильном формате...?\r\n";
				$this->isErrorElement = true;
            }
        } else {
...
[s].[uid])) AND ([s].version_end = ".$this->version_end.")
                    WHERE
					  ...
					  AND [pat].birthday_38523 = '".implode('', explode('-', $oms->birthday))."'";
    ...
    private function addTasuPatient($medcard, $oms) {
		...
		try {
            ...
            if($number) { // Номер в любом случае должен быть
                ...
                if($smoRow == null) {
                    $this->log[] = '<strong class="text-danger">[Ошибка]</strong> Невозможно найти страховую компанию с кодом региона '.$regionCode.' и кодом СМО '.$smoCode.' у пациента с номером карты '.$medcard->card_number.'!';
                    $this->logStr .= '[Ошибка] Невозможно найти страховую компанию с кодом региона '.$regionCode.' и кодом СМО '.$smoCode.' у пациента с номером карты '.$medcard->card_number.'!\r\n';
                    return false;
                }
			...
			$this->addTasuDul($medcard, $oms, $patientRow['PatientUID']);
	...
	private function addTasuDul($medcard, $oms, $patientUID) {
		$conn = Yii::app()->db2;
    ...
    private function addTasuTap($patient, $greeting, $oms, $medcard) {
        ...
        try {
			...
			if($policyRow == null) {
                /*  $this->log[] = '<strong class="text-danger">[Ошибка]</strong> Невозможно найти документ для пациента с ID '.$patient['PatientUID'].' (карта '.$medcard->card_number.', '.$greeting['patient_day'].')!';
				$this->logStr .= "[Ошибка] Невозможно найти документ для пациента с ID ".$patient['PatientUID']." (карта ".$medcard->card_number.", ".$greeting['patient_day'].")!r\n";
                $this->isErrorElement = true; */
				$this->addTasuDul($medcard, $oms, $patient['PatientUID']);
            }
...
//		}
	...
	public function actionSendLogFile() {
		if(isset($_GET['bufferid'], $_GET['type'])) {
            if($_GET['type'] == 0) {
                $bufferH = TasuMedcardsBufferHistory::model()->findByPk($_GET['bufferid']);
            } else if ($_GET['type'] == 1) {
	...
	public function actionCancelImport() {
		if(isset($_GET['bufferid'], $_GET['type'])) {
            if($_GET['type'] == 0) {
                $bufferHModel = TasuGreetingsBufferHistory::model()->findByPk($_GET['bufferid']);
                $bufferModel = TasuGreetingsBuffer::model();
            } else if($_GET['type'] == 1) {
                $bufferHModel = TasuMedcardsBufferHistory::model()->findByPk($_GET['bufferid']);
                $bufferModel = TasuMedcardsBuffer::model();
            }

            // Обязательно переставить importId на максимальный, чтобы отображать в списке прошлые выгрузки
            $lastImportId = $bufferModel->getLastImportId();
            $bufferModel->updateAll(array(
                    'status' => 0,
                    'import_id' => $lastImportId['max_import_id']
                ),
                'import_id = :import_id',
                array(':import_id' => $bufferHModel->import_id)
            );

			$bufferHModel->status = 2; // Статус "отменена"
			$bufferHModel->save();
		}
		echo CJSON::encode(array(
			'success' => true,
    ...
    public function actionGetBufferMedcards() {
        ...
        $num = $model->getLastBuffer($filters, false, false, false, false, $importId);

        $totalPages = ceil(count($num) / $rows);
        $start = $page * $rows - $rows;

        $buffer = $model->getLastBuffer($filters, $sidx, $sord, $start, $rows, $importId);
    ...
    private function getHistoryGridByModel($model) {
        $rows = $_GET['rows'];
        $page = $_GET['page'];
        $sidx = $_GET['sidx'];
        $sord = $_GET['sord'];
        ...
        foreach($buffer as &$element) {
            ...
            if($element['log_path'] != null) {
                $type = (int)($model instanceof TasuGreetingsBufferHistory);
                $element['log'] = '<a href="/admin/tasu/sendlogfile/?type='.$type.'&bufferid=' . $element['id'] . '" target="_blank" id="l' . $element['id'] . '"><span class="glyphicon glyphicon-download-alt"></span></a>';
            } else {
                $element['log'] = '-';
            }
        }
    ...
    public function actionGetBufferMedcardsHistory() {
        $this->getHistoryGridByModel(new TasuMedcardsBufferHistory());
    }
    ...
    public function actionImportMedcards() {
        // Делаем выборку чанка приёмов с целью последующей выгрузки
        $currentMedcard = isset($_GET['currentMedcard']) && $_GET['currentMedcard'] != null ? $_GET['currentMedcard'] : false;
        $limit = isset($_GET['rowsPerQuery']) ? $_GET['rowsPerQuery'] : false;
        $sord = 'asc';
        $sidx = 'buffer_id';
        $start = 0;

        $buffer = TasuMedcardsBuffer::model()->getLastBuffer(false, $sidx, $sord, $start, $limit, false, $currentMedcard);
        ...
        if(isset($_GET['totalRows']) && $_GET['totalRows'] == null) {
            $rows = TasuMedcardsBuffer::model()->getLastBuffer(false);
            $this->totalRows = count($rows);
            // Это прогон в первый раз. Если totalRows = 0, то заданий нет, это ошибка
            if($this->totalRows == 0) {
                echo CJSON::encode(array(
                    'success' => false,
                    'error' => 'Нет медкарт для выгрузки!'
                ));
                exit();
            }
        } elseif(isset($_GET['totalRows'])) {
        ...
        foreach($buffer as $element) {
            ...
            $this->moveMedcardToTasuDb($element);
            $bufferMedcardModel = TasuMedcardsBuffer::model()->findByPk($element['buffer_id']);
            if($bufferMedcardModel && !$this->isErrorElement) {
                $bufferMedcardModel->status = 1;
                if(!$bufferMedcardModel->save()) {
                    $this->log[] = '<strong class="text-danger">[Ошибка]</strong> Невозможно изменить статус приёма в буфере c ID'.$element['buffer_id']." (карта ".$element['medcard'].")";
                    $this->logStr .= "[Ошибка] Невозможно изменить статус приёма в буфере c ID".$element['buffer_id']." (карта ".$element['medcard'].")\r\n";
                    $this->isErrorElement = true;
                } else {
                    $this->log[] = '<strong class="text-success">[OK]</strong> Статус приёма в буфере c ID'.$element['buffer_id']." (карта ".$element['medcard'].") успешно изменён.";
                    $this->logStr .= "[OK] Статус приёма в буфере c ID".$element['buffer_id']." (карта ".$element['medcard'].") успешно изменён.\r\n";
                }
            } else {
                $this->isErrorElement = false;
            }
            $this->lastId = $element['buffer_id'];
        }
        ...
        // Делаем контрольный запрос. Если на выборку контрольного запроса выбирается 0 строк, то, значит, это окончание выгрузки. Надо перенести выгрузку в историю
        $moreBuffer = TasuMedcardsBuffer::model()->getLastBuffer(false, $sidx, $sord, $start, $limit, $this->lastId);
        if(count($moreBuffer) == 0) {
            // Найдём все невыгруженные записи (ошибки. Это количество отнять от общего)
            $numErrors = TasuMedcardsBuffer::model()->count(
                'import_id = :import_id
				AND status = :status',
                array(
                    ':import_id' => $importId,
            ...
            $historyBuffer = new TasuMedcardsBufferHistory();
            $historyBuffer->num_rows = $this->totalRows - $numErrors;
            $historyBuffer->create_date = date('Y-m-d h:i');
            $historyBuffer->status = 1; // Выгружено
            $historyBuffer->import_id = $importId;
            ...
            // Обновим importId для невыгруженных
            TasuMedcardsBuffer::model()->updateAll(
                array('import_id' => $importId + 1),
                'import_id = :import_id
				AND status = :status',
                array(
    ...
    public function moveMedcardToTasuDb($bufferMedcard) {
        $medcard = Medcard::model()->findByPk($bufferMedcard['medcard']);
        $oms = Oms::model()->findByPk($medcard['policy_id']);
        ...
        if(count($patients) == 0 || !$patients) {
            ...
            if($result === false) {
                $this->numErrors++;
                $this->log[] = '<strong class="text-danger">[Ошибка]</strong> Невозможно добавить пациента с ОМС '.$oms->oms_number.' ('.$oms->last_name.' '.$oms->first_name.' '.$oms->middle_name.'), карта '.$bufferMedcard['medcard'];
                $this->logStr .= "[Ошибка] Невозможно добавить пациента с ОМС ".$oms->oms_number." (".$oms->last_name." ".$oms->first_name." ".$oms->middle_name."), карта ".$bufferMedcard['medcard'].")\r\n";
                $this->isErrorElement = true;
                return false;
            } else {
                $this->numAddedPatients++;
    ...
    private function createMedcardInTasu($medcard, $patientUID) {
        $conn = Yii::app()->db2;
        // Создать медкарту
        // Максимальный UID
        $conn = Yii::app()->db2;
    ...
    private function updateMedcardInTasu($medcard, $patientUID) {
        $conn = Yii::app()->db2;
        // Обновим запись о медкарте в ТАСУ
        $sql = "ALTER TABLE dbo.t_book_65067 DISABLE TRIGGER trt_book_65067_update";
        $conn->createCommand($sql)->execute();
