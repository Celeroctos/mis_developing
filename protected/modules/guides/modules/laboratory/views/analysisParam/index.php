<script type="text/javascript">
    globalVariables.guideEdit = '<?php echo Yii::app()->user->checkAccess('guideEditAnalysisParam'); ?>';</script>
...
function() {
    ...
    $.get(url, function(r){
		...
		$("#DialogCRUDForm").html(r).dialog("option", "title", "Редактирование параметра анализа").dialog("open");
    });
    return false;
}
EOT;
...
#		'comment',
        array(
            'class' => 'CButtonColumn',
            'deleteConfirmation' => "js:'Вы уверены, что хотите удалить параметр анализа \'' + $(this).parent().parent().children(':nth-child(1)').text() + '\'?'",
            'template' => $template,
            'buttons' => $buttons,
        ),
    ),
