misEngine['class']('component.module.hospital.armd', function(){
	return {
		...
		displayOperationsGrid: function(){
			...
			console.debug('displayOperationsGrid');
			//$('#armd-operations').html('operations');
			var operationsGrid = misEngine.create('component.grid');
			operationsGrid.setConfig({
				id: 'operationsGrid'
			});
			var expanderBody = $('.operationsExpanderBody');
		...
		displayDesignationsGrid: function(){
			console.debug('displayDesignationsGrid');
			...
			this.myPatientsGrid.setConfig({
				...
				renderConfig: {
					...
					ajaxConf: {
						...
						success: function(data, status, jqXHR){
							if (data.success) {
								$(myPatientsGridRequestData.container).css({
									'textAlign': 'left'
								}).html(data.data);
							}
						},
				...
				renderConfig: {
					...
					ajaxConf: {
						...
						success: function(data, status, jqXHR){
							if (data.success) {
								if (data.num > 0) {
									$('#armdMyPatientsTabmark .roundedLabelText').text(data.num).parent().css('display', 'inline')
								}
								else {
				...
				renderConfig: {
					...
					ajaxConf: {
						...
						success: function(data, status, jqXHR){
							if (data.success) {
								if (data.num > 0) {
									$('#armdPatientsTabmark .roundedLabelText').text(data.num).parent().css('display', 'inline');
								}
								else {
				...
				renderConfig: {
					...
					ajaxConf: {
						...
						success: function(data, status, jqXHR){
							if (data.success) {
								if (data.num > 0) {
									$('#armdOperationsTabmark .roundedLabelText').text(data.num).parent().css('display', 'inline');
								}
								else {
			...
			$(document).on('click', '.desktopTab li.last', $.proxy(function(e){
				...
				var popover = $(e.target).parent().popover({
					...
					content: $.proxy(function(){
						return $('<div>').append(this.getTemplatesList(e.target), $('<button>').prop({
							'class': 'btn btn-success'
						}).text('OK').on('click', $.proxy(function(e){
		...
		getTemplatesList: function(container){
			var ul = $('<ul>').append($('<li>').addClass('list-group-item').append($('<input>').prop({
				'type': 'checkbox',
				'class': 'template-choose'
			}), $('<a>').text('Шаблон 1').prop({
				'href': '#'
			})), $('<li>').addClass('list-group-item').append($('<input>').prop({
				'type': 'checkbox',
				'class': 'template-choose'
 ...
 closeCross);
		...
		displayDiagnosisFields: function(){
			this.preClinicalDiagnosis = misEngine.create('component.livesearch', {
				...
				'extraparams': { //'region' : $.fn['regionChooser'].getChoosed(),
